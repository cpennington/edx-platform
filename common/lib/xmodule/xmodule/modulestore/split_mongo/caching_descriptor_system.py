...
class CachingDescriptorSystem(MakoDescriptorSystem, EditInfoRuntimeMixin):
    ...
    def _load_item(self, usage_key, course_entry_override=None, **kwargs):
        ...
        new_item = self.xblock_from_json(class_, course_key, block_key, json_data, course_entry_override, **kwargs)
        return new_item
    ...
    def xblock_from_json(
        ...
        if block_key in self._parent_map:
            ...
            parent = self.course_entry.course_key.make_usage_key(parent_key.type, parent_key.id)
        else:
            parent = None
        kvs = SplitMongoKVS(
            definition_loader,
